        ...
        try:
            ...
            for index, holder in enumerate(cdsl_utility.kyc_data):
                ...
                _holder = HolderRecord(
                    ...
                    ScndHldrNmOfFthr=cdsl_utility.father_or_husband_name_value(
                        index=index
                    ),
            ...
            try:
                response = CDSLDematPayloadModel(
                    records=all_entries, signatures=signatures
                )
                return response
            except ValidationError as e:
                raise PluginException(
                    "CDSL Demat Payload Model Validation Failed",
                    detailed_message=str(e),
                )
        except Exception as e:
            raise PluginException("CDSL Demat payload parsing error", str(e))
