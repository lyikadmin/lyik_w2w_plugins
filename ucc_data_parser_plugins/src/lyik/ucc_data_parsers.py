...
class UCCDataParser(UCCDataParserSpec):
    ...
    async def ucc_data_parse(
        ...
        try:
            ...
            depository = (
                _form_record.get("dp_information", {})
                .get("dp_Account_information", {})
                .get("depository", "")
            )
            if depository == "NSDL":
                return await self.ucc_nse_data_parse(
                    context=context, form_record=form_record
                )
            elif depository == "CDSL":
                return await self.ucc_bse_data_parse(
                    context=context, form_record=form_record
                )
            else:
                raise PluginException("Depository value not found, cannot proceed!")
        except Exception as e:
            raise PluginException("Failed to parse UCC data", str(e))
    ...
    async def ucc_bse_data_parse(
        ...
        try:
            ...
            kyc_data = (
                _form_record.get("kyc_holders", [])[0].get("kyc_holder", {})
                if 0 < len(_form_record.get("kyc_holders", []))
                else {}
            )
            ...
            raise PluginException("Failed to parse BSE data", str(e))
    ...
    async def ucc_nse_data_parse(
        ...
        try:
            ...
            if (
                str(
                    _form_record.get("application_details", {}).get(
                        "kyc_digilocker", ""
                    )
                ).lower()
                != "no"
                or str(
                    kyc_data.get("identity_address_verification", {})
                    .get("ovd", {})
                    .get("ovd_type", "")
                ).lower()
                == "aadhaar"
            ...
            NSE_MEMERCODE = (
                "11502"  # todo: this should come from env! same value as username
            )
            base_nse_data = NSEEntry(
                ...
                ccdPermAddLine1=(
                    nse_utility.permanent_address_value()
                    if is_permanent_address_same != "Y"
                    else ""
                ),
                ccdPermAddLine2="",  # optional
                ccdPermAddLine3="",  # optional
                ccdPermAddCity=(
                    nse_utility.permanent_address_city_value()
                    if is_permanent_address_same != "Y"
                    else ""
                ),
                ccdPermAddState=(
                    nse_utility.permanent_address_state_value()
                    if is_permanent_address_same != "Y"
                    else ""
                ),
                ccdPermAddCountry=(
                    nse_utility.permanent_address_country_value()
                    if is_permanent_address_same != "Y"
                    else ""
                ),
                ccdPermPin=(
                    nse_utility.permanent_address_pincode_value()
                    if is_permanent_address_same != "Y"
                    else ""
                ),
                ccdOffcIsd="",  # optional
            ...
            segment_pref_1 = nse_utility.segment_indicator_value(
                segment=_form_record.get("trading_information", {})
                .get("trading_account_information", {})
                .get("segment_pref_1", "")
            )
            segment_pref_2 = nse_utility.segment_indicator_value(
                segment=_form_record.get("trading_information", {})
                .get("trading_account_information", {})
                .get("segment_pref_2", "")
            )
            segment_pref_3 = nse_utility.segment_indicator_value(
                segment=_form_record.get("trading_information", {})
                .get("trading_account_information", {})
                .get("segment_pref_3", "")
            )
            segment_pref_4 = nse_utility.segment_indicator_value(
                segment=_form_record.get("trading_information", {})
                .get("trading_account_information", {})
                .get("segment_pref_4", "")
            )
            segment_pref_5 = nse_utility.segment_indicator_value(
                segment=_form_record.get("trading_information", {})
                .get("trading_account_information", {})
                .get("segment_pref_5", "")
            )
            segment_pref_6 = nse_utility.segment_indicator_value(
                segment=_form_record.get("trading_information", {})
                .get("trading_account_information", {})
                .get("segment_pref_6", "")
            )
            ...
            if segment_pref_6:  # SLB
                ...
                slb_nse_entry = base_nse_data.model_copy(
                    update={"ccdSegInd": segment_pref_6}
                )
                nse_entries.append(slb_nse_entry)

            return NSEPayload(nse_entries=nse_entries)
        except Exception as e:
            raise PluginException("Failed to parse NSE data", str(e))
