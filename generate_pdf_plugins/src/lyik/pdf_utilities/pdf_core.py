...
class PdfCore:
    ...
    """
    async def generate_all_docs(self, context: ContextModel,form_record: GenericFormRecordModel,record_id:int)->GenerateAllDocsResponseModel:
        """
        ...
        try:
            ...
            if temp_name:
                ...
                existing_pdfs:List[DBDocumentModel] = await self._get_files_from_db(context=context,query_params=DocQueryGenericModel(org_id=context.org_id,form_id=context.form_id,record_id=record_id))
    ...
    async def _get_files_from_db(self,context:ContextModel,doc_id:str|None=None,query_params: DocQueryGenericModel| None=None)->List[DBDocumentModel] | None:
        '''
        ...
        try:
            if docs and len(docs)>0:
                ...
                for existing_pdf in docs:
                    if existing_pdf.metadata.esign and existing_pdf.metadata.esign.esign_meta:
                        for em in existing_pdf.metadata.esign.esign_meta:
                            if em.sign_status.status==EsignState.DONE or em.sign_status.status==EsignState.INPROGRESS:
                                return True
        except Exception as e:
            return False
